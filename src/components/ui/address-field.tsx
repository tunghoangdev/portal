...
export const AddressSelects = <TFormValues extends FieldValues>({
	...
	const { setValue, getValues, reset, trigger } = formMethods || {};
	...
	const { getAll: getAllWard, isFetching: isLoadingWards }: any = useCrud(
		[API_ENDPOINTS.dic.commune, watchedDistrictId || ''],
		{
			endpoint: '',
	...
	useEffect(() => {
		...
		if (watchedDistrictId && !isCurrentWardValid) {
			setValue?.(wardFieldName as Path<TFormValues>, '' as any, {
				shouldValidate: true,
			});
		} else if (!watchedDistrictId) {
			setValue?.(wardFieldName as Path<TFormValues>, '' as any, {
				shouldValidate: true,
			});
		}
	}, [watchedDistrictId, wardOptions, setValue, wardFieldName, control]);
	...
	useEffect(() => {
		if (formData) {
			...
			reset?.(newFormData);
			trigger?.([wardFieldName, districtFieldName, provinceFieldName]);
		}
	}, [formData, wardOptions, districtOptions, provinceOptions]);

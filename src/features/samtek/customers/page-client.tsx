...
import { ConfigBusiness } from "./config-business";
...
const columns = getColumns<any>(listColumns, {
  actions: [
    ...
    CRUD_ACTIONS.SAMTEK_CONFIG_BUSINESS,
    CRUD_ACTIONS.LOG,
    CRUD_ACTIONS.EDIT,
    ...
    async (action: CrudActionType, formData?: TItemFormFields) => {
      if (action === CRUD_ACTIONS.CONFIG_LEVEL) {
        openDetailModal(formData, {
          title: `Quản lý cấp bậc - ${formData?.company_name}`,
          renderContent: () => <ConfigLevel data={formData} />,
        });
        return;
      }
      if (action === CRUD_ACTIONS.SAMTEK_CONFIG_BUSINESS) {
        openDetailModal(formData, {
          title: `Quản lý nghiệp vụ - ${formData?.company_name}`,
          renderContent: () => <ConfigBusiness data={formData} />,
        });
        return;
      }
      ...
      if (
        action === CRUD_ACTIONS.LOCK_ACCESS ||
        action === CRUD_ACTIONS.UNLOCK_ACCESS
      ) {
  ...
  return (
    <DataTable
      ...
      customActions={[
        ...
        {
          type: CRUD_ACTIONS.SAMTEK_CONFIG_BUSINESS,
          label: "Quản lý nghiệp vụ",
          icon: <Icons.folderConfig size={16} />,
        },
        {

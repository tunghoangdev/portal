...
import { formLevelFields } from './form-level.schema';
import { FormField } from '~/features/shared/components/form-fields';

interface FormLevelViewProps<T extends FieldValues> {
	control: Control<T>;
	action: ToolbarAction;
	formMethods?: UseFormReturn<T>;
}

export function FormLevelView<T extends FieldValues>({
	control,
	formMethods,
}: FormLevelViewProps<T>) {
	return (
		<Grid container spacing={4}>
			{formLevelFields.map((field: any, k: number) => {
...
FormLevelView.displayName = 'FormLevelView';

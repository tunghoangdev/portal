import { useCallback, useEffect, useMemo, useState } from 'react'; // Thêm useRef, useCallback
...
export default function ListProcessingPageClient() {
	...
	const basePath = API_ENDPOINTS[role].individual.nonLifeInsurance;
	const basePath2 = API_ENDPOINTS[ROLES.STAFF].nonLifeInsurance.processing;
			...
			customActions={[
				{
					...
					isHidden: (row: any) => {
						return row?.id_none_life_status !== 3;
					},
				},
				{
					...
					isHidden(row: any) {
						return row?.id_none_life_status > 1;
					},
				},
				{

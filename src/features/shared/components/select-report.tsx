...
import { useAuth, useCrud } from '~/hooks';
...
export default function SelectReport({ label, name, placeholder }: IProps) {
	const [selectedReport, setSelectedReport] = useState<any>();
	const { role } = useAuth();
	const { setData } = useCommonStore();
	const { data: reports } = useCommonData(
		'reportingPeriod',
		API_ENDPOINTS.common.getListPeriod,
		{
			fetchOnInit: true,
		},
	);
	...
	const newList =
		role === ROLES.STAFF
			? reports
			: reports?.filter((report: any) => report.id !== 30);
	return (
		<Autocomplete
			...
			classNames={{
				...
				base: 'min-w-[100px] md:min-w-[220px]',
			}}
			popoverProps={{
				radius: 'sm',
				classNames: {
			...
			inputProps={{
				classNames: {
					inputWrapper:
						'border border-default-400 min-h-8 h-8 md:min-h-9 md:h-9 bg-white',
					label: 'text-black/90 top-[20px] font-medium',
					input: 'text-xs md:text-[13px] !shadow-none text-foreground-500',
